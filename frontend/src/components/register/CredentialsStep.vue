...
<template>
  <div class="space-y-6">
    ...
    <div class="space-y-4">
      ...
      <div>
        ...
        <div class="mt-1 relative">
          <input id="password" :type="showPassword ? 'text' : 'password'" v-model="password" :class="[
            'block w-full px-4 py-3 rounded-xl border bg-white dark:bg-gray-900 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all duration-200',
            errors.password
              ? 'border-red-300 dark:border-red-700'
              : 'border-gray-300 dark:border-gray-700'
        ...
        <div class="mt-4 space-y-2">
          ...
          <div class="space-y-1">
            <div v-for="(req, index) in passwordRequirementsMet" :key="index" class="flex items-center space-x-2">
              <div :class="[
    ...
    <!-- Navigation Buttons -->
    <NavigationButtons :is-valid="isFormValid" :show-next="true" :show-previous="true" next-text="Siguiente"
      @next="$emit('valid')" @previous="$emit('prev')" />
  </div>
</template>
