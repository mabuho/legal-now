          ...
          <div v-for="c in pendingList" :key="c.id" :class="['mx-3 mb-2 p-3 rounded-card cursor-pointer transition-all border',
            ...
            <div class="flex justify-between items-start gap-2">
              <div class="min-w-0">
                ...
                <p class="font-body text-xs text-text-muted mt-0.5">{{ c.client?.name ?? '—' }} · {{
                  formatDate(c.created_at) }}</p>
              </div>
              <span
      ...
      <template v-else>
        ...
        <div class="px-6 py-4 border-b border-border-subtle bg-surface-raised flex items-start justify-between gap-4">
          <div class="min-w-0">
            ...
            <p class="font-body text-sm text-text-muted mt-0.5">{{ selectedDetail.client?.name ?? '—' }} · {{
        ...
        <div class="p-6 flex-1 overflow-y-auto">
          ...
          <div class="bg-surface-card border border-border-default rounded-card p-4">
            ...
            <div class="flex items-center gap-3">
              ...
              <div>
                <p class="font-heading font-semibold text-sm text-text-primary">{{ selectedDetail.client?.name ?? '—' }}
                </p>
              </div>
            </div>
          </div>
