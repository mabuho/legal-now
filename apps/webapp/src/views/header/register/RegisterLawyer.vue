<template>
  <form @submit.prevent="handleSubmit" class="space-y-8" novalidate>
    <div class="space-y-6" role="group" aria-labelledby="personal-info-heading">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div class="relative">
          <input v-model="formData.firstName" type="text" id="firstName"
            ...
            :class="{ 'border-red-500': errors.firstName }" placeholder="Nombre" aria-required="true"
          ...
          <p v-if="errors.firstName" id="firstName-error" class="mt-1 text-sm text-red-500" role="alert">
            {{ errors.firstName }}
          </p>
        </div>
        ...
        <div class="relative">
          <input v-model="formData.lastName" type="text" id="lastName"
            class="peer h-12 w-full border-b-2 border-gray-300 text-gray-900 placeholder-transparent focus:outline-none focus:border-indigo-600 bg-transparent focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2"
            :class="{ 'border-red-500': errors.lastName }" placeholder="Apellido" aria-required="true"
            :aria-invalid="errors.lastName ? true : false" aria-describedby="lastName-error" />
          <label for="lastName"
            class="absolute left-0 -top-3.5 text-gray-600 text-sm transition-all peer-placeholder-shown:text-base peer-placeholder-shown:text-gray-400 peer-placeholder-shown:top-3 peer-focus:-top-3.5 peer-focus:text-gray-600 peer-focus:text-sm">
            Apellido
          </label>
          <p v-if="errors.lastName" id="lastName-error" class="mt-1 text-sm text-red-500" role="alert">
            {{ errors.lastName }}
          </p>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div class="relative">
          <input v-model="formData.email" type="email" id="email"
            ...
            :class="{ 'border-red-500': errors.email }" placeholder="Correo Electrónico" />
          <label for="email"
            class="absolute left-0 -top-3.5 text-gray-600 text-sm transition-all peer-placeholder-shown:text-base peer-placeholder-shown:text-gray-400 peer-placeholder-shown:top-3 peer-focus:-top-3.5 peer-focus:text-gray-600 peer-focus:text-sm">
            Correo Electrónico
          </label>
        ...
        <div class="relative">
          <input v-model="formData.phone" type="tel" id="phone"
            class="peer h-12 w-full border-b-2 border-gray-300 text-gray-900 placeholder-transparent focus:outline-none focus:border-indigo-600 bg-transparent"
            :class="{ 'border-red-500': errors.phone }" placeholder="Teléfono" />
          <label for="phone"
            class="absolute left-0 -top-3.5 text-gray-600 text-sm transition-all peer-placeholder-shown:text-base peer-placeholder-shown:text-gray-400 peer-placeholder-shown:top-3 peer-focus:-top-3.5 peer-focus:text-gray-600 peer-focus:text-sm">
            Teléfono
          </label>
      ...
      <div class="relative">
        <input v-model="formData.licenseNumber" type="text" id="licenseNumber"
          class="peer h-12 w-full border-b-2 border-gray-300 text-gray-900 placeholder-transparent focus:outline-none focus:border-indigo-600 bg-transparent"
          :class="{ 'border-red-500': errors.licenseNumber }" placeholder="Número de Colegiatura" />
      ...
      <div class="relative">
        <div class="select-menu" id="specialization" :class="{ active: isActive }" @keydown.esc="isActive = false"
          ...
          @keydown.up.prevent="navigateOptions('up')" @keydown.down.prevent="navigateOptions('down')" tabindex="0"
          role="combobox" aria-haspopup="listbox" :aria-expanded="isActive" aria-labelledby="specialization-label"
          ...
          <div class="select-btn" @click="toggleDropdown">
            ...
            <ChevronUpDownIcon class="h-5 w-5 text-gray-500 transition-transform duration-300"
              :class="{ 'rotate-180': isActive }" aria-hidden="true" />
          </div>
          <ul id="specialization-options" class="options" role="listbox"
            :aria-activedescendant="focusedOption ? `option-${focusedOption.value}` : undefined">
            <li v-for="option in specializationOptions" :key="option.value" :id="'option-' + option.value"
              class="option" :class="{ 'bg-indigo-50': focusedOption?.value === option.value }"
              @click="selectOption(option)" role="option" :aria-selected="selectedOption === option.label">
              <component :is="option.icon" class="h-5 w-5" :class="option.iconClass" aria-hidden="true" />
              <span class="option-text">{{ option.label }}</span>
            </li>
          </ul>
        </div>
        <p v-if="errors.specialization" id="specialization-error" class="mt-1 text-sm text-red-500" role="alert">
      ...
      <div class="relative">
        <div class="relative">
          <input v-model="formData.password" :type="showPassword ? 'text' : 'password'" id="password"
            ...
            :class="{ 'border-red-500': errors.password }" placeholder="Contraseña" aria-label="Password input" />
          <label for="password"
            class="absolute left-0 -top-3.5 text-gray-600 text-sm transition-all peer-placeholder-shown:text-base peer-placeholder-shown:text-gray-400 peer-placeholder-shown:top-3 peer-focus:-top-3.5 peer-focus:text-gray-600 peer-focus:text-sm">
            Contraseña
          </label>
          <button type="button" @click="showPassword = !showPassword"
            class="absolute inset-y-0 right-0 flex items-center pr-3" aria-label="Toggle password visibility">
        ...
        <div class="mt-2" v-if="formData.password">
          ...
          <ul class="mt-2 text-sm text-gray-600 space-y-1">
            <li class="flex items-center gap-2">
              <div class="w-2 h-2 rounded-full" :class="formData.password.length >= 8 ? 'bg-green-500' : 'bg-gray-300'">
              </div>
              Mínimo 8 caracteres
            </li>
            <li class="flex items-center gap-2">
              <div class="w-2 h-2 rounded-full"
                :class="/[A-Z]/.test(formData.password) ? 'bg-green-500' : 'bg-gray-300'"></div>
              Una mayúscula
            </li>
            <li class="flex items-center gap-2">
              <div class="w-2 h-2 rounded-full"
                :class="/[0-9]/.test(formData.password) ? 'bg-green-500' : 'bg-gray-300'"></div>
              Un número
            </li>
            <li class="flex items-center gap-2">
              <div class="w-2 h-2 rounded-full"
                :class="/[^A-Za-z0-9]/.test(formData.password) ? 'bg-green-500' : 'bg-gray-300'"></div>
      ...
      <div class="relative">
        <div class="relative">
          <input v-model="formData.confirmPassword" :type="showConfirmPassword ? 'text' : 'password'"
            id="confirmPassword"
            class="peer h-12 w-full border-b-2 border-gray-300 text-gray-900 placeholder-transparent focus:outline-none focus:border-indigo-600 bg-transparent pr-10"
            :class="{ 'border-red-500': errors.confirmPassword }" placeholder="Confirmar Contraseña"
          ...
          <button type="button" @click="showConfirmPassword = !showConfirmPassword"
            class="absolute inset-y-0 right-0 flex items-center pr-3" aria-label="Toggle confirm password visibility">
        ...
        <div class="relative flex items-start">
          <div class="flex items-center h-5">
            <input type="checkbox" v-model="formData.terms" id="terms"
              ...
              aria-required="true" :aria-invalid="errors.terms ? true : false" aria-describedby="terms-error" />
          </div>
